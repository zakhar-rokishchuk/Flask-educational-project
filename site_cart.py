...
from data_manipulations import create_order_from_cart, get_product
from notifications import get_notifications, add_notification
import cart_manipulations
...
def cart_list():
    if cart_manipulations.has_order():
        return render_template('cart.html', order=cart_manipulations.get_order(), notifications=get_notifications(), sum=cart_manipulations.get_order_sum())
...
def ordering():
    if cart_manipulations.has_order():
        return render_template('ordering.html', order=cart_manipulations.get_order(), notifications=get_notifications(), sum=cart_manipulations.get_order_sum())
    return render_template('ordering.html', order=[], notifications=get_notifications())


@site_cart.route('/create_order', methods=['POST'])
def create_order():
    create_order_from_cart(request.form, cart_manipulations.get_order())
