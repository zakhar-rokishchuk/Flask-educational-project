...
from data_manipulations import add_comment_to_order, filter_orders_by_name, filter_orders_by_status, get_orders, save_orders, get_order, sort_orders_by_date, save_order
import json
...
def orders():
    if request.args.get("filter_orders"):
        return render_template('orders.html', orders=filter_orders_by_status(request.args.get("filter_orders")))
    if request.args.get("search_name"):
        return render_template('orders.html', orders=filter_orders_by_name(request.args.get("search_name")))
    return render_template('orders.html', orders=sort_orders_by_date())


@admin_orders.route('/admin/orders/<int:order_id>', methods=["GET"])
def order(order_id):
    return render_template('order.html', order=get_order(order_id))
...
def edit_order(order_id):
    return render_template('editing_order.html', order=get_order(order_id))
