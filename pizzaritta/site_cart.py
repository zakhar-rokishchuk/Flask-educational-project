...
def add_to_cart(item_id):
    item = db_products_manipulations.get_product(item_id)
    if not cart_manipulations.has_current_order():
...
def cart_list():
    if cart_manipulations.has_current_order():
        return render_template('cart.html', order=cart_manipulations.get_current_order(), notifications=notifications.get_notifications(), sum=cart_manipulations.get_order_sum())
...
def ordering():
    if cart_manipulations.has_current_order():
        return render_template('ordering.html', order=cart_manipulations.get_current_order(), notifications=notifications.get_notifications(), sum=cart_manipulations.get_order_sum())
...
def create_order():
    cart_manipulations.create_order_from_cart(request.form, cart_manipulations.get_current_order())
    cart_manipulations.clear_session()
    return redirect("/")
